        ...
        <table class="table table-bordered">
            ...
            <?php
                if(!empty($_SESSION["depts"]))
                    {
                        ...
                        ('
                        SELECT i.name , i.total , r.ranking FROM info i , covering c , pmp p , hipo h , ranking r WHERE
                ...
                elseif(empty($_SESSION["depts"]))
                        {
                            $select_total_name = $connect_database->prepare('SELECT name , total , ranking FROM ranking ORDER BY ranking');
                            $select_total_name->execute();
                        }
                            ...
                            echo '
                                    <tr>
                                        <td width="750px">
                                            ...
                                            <label class="btn btn-outline-secondary" for="'.$print["name"].'"> '.$print["ranking"].' - '.$print["name"].' </label>
                                            <div class="progress">
